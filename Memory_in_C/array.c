...
int* bubbleSort(int arr[], int n) {
    ...
    // Your code goes here...
    // need to edit this using *(arr + i) notation
    for (i = 0; i < n - 1; i++) {
        for(j = 0; j < n - 1; j++) {
            if(*(s_arr + j) > *(s_arr + j + 1)) {
                temp = *(s_arr + j + 1);
...
int bSearch(int *arr, int a, int b, int key) {
    ...
    if (a <= b){    
        ...
        // compare value at midpoint to key
        if (*(arr + midpoint) == key){
            return midpoint;
        }

        // if key is greater than *(arr + midpoint - 1), search second half of subarray
        if (key > *(arr + midpoint)){
            return bSearch(arr, midpoint + 1, b, key);
        }

        // else if key is less than *(arr + midpoint - 1), search first half of subarray
        if (key < *(arr + midpoint)){
            return bSearch(arr, a, midpoint - 1, key);
        }
    }
